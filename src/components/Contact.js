...
export const Contact = () => {
    ...
    const [formDetails, setFromDetails] = useState(formDetails);
    const [buttonText, setButtonText] = useState('Send');
    const [status, setStatus] = useState({});

    const onFormUpdated = (category, value) => {
        setFromDetails({
            ...formDetails,
            [category]: value
        })
    ...
    const handleSubmit = async (event) => {
        ...
        if (result.code === 200) {
            ...
            setStatus({ success: false, message: 'Somenthing went wrong, please try again later.' });
        }
    }
                        ...
                        <form onSubmit={handleSubmit}>
                            <Row>
                                <Col sm={6} className="px-1">
                                    <input type={text} value={formDetails.firtsName} placeholder="First Name" onChange={(e) => onFormUpdate('firstName', e.target.value)} />
                                </Col>
                                <Col sm={6} className="px-1">
